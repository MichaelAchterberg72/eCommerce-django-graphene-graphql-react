...
function Dashboard(props){
    ...
    const handleSubmit = async (tokenAccess) => {
        ...
        // if success get update user information
        ...
        customNotifier({
            type: "success",
            content: "Your Business has been successfully created",
        });
        setModalState(false);
        setLoading(false);
    };
                ...
                modalState && (
                    <ModalComponent onClose={() => setModalState(false)}>
                        <div className={styles.businessContainer}>
                            ...
                            <button type="submit" disabled={loading} className={loading ? "loading disabled" : ""}>SUBMIT BUSINESS</button>
                            </form>
                        </div>
                    </ModalComponent>
                )
...
const ModalComponent = ({ children, onClose }) => {
    return (
        <div className="modal">
            <div className="inner">
                <img src="/close.png" className="close" onClick={onClose} />
                { children }
            </div>
        </div>
    );
